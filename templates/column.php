  ...
  <div class="entry <?php if($now) echo 'now'; ?>" id="<?php echo $id?>">
    ...
    <?php foreach($posts as $post):?>
        <div class="sub_entry" id="<?php echo $post['money_id']?>">
          ...
          <?php if($post['user_id'] !== ''):?>
            <div class="edit left" title="Eintrag bearbeiten" 
                 onClick="edit(<?php echo $post['money_id']?>); return false;">
            </div>
	          <div class="delete left" title="Eintrag löschen" 
	               onClick="deleteEntry(<?php echo $post['money_id'] ?>); return false;">
	          </div>
          <?php endif ?>
		    </div>
	  ...
	  <?php endforeach?>				    
	  <div class="monthly-sum">Summe: &euro;&nbsp;<span style="font-size:16px"><?php echo $this->sums[$month]?></span></div>
